                        ...
                        if x_info:
                            ...
                            if modulation_info and 'lora' in modulation_info:
                                ...
                                if lora_modulation_info:
                                    codr = lora_modulation_info.get('codeRate', None)
                                    if codr is not None:
                                        standard_packet['codr'] = codr
                                    spreading_factor = lora_modulation_info.get('spreadingFactor')
                                    bandwidth = lora_modulation_info.get('bandwidth')
                                    if spreading_factor is not None:
                                        standard_packet['datr'] = json.dumps({
                                            "spread_factor": spreading_factor,
                                            "bandwidth": bandwidth
                                        })
